...
public class OfflineConntroller {
    ...
    @ApiOperation(value="核销订单", notes="根据opendid和单号,收银员编号,收银员名称 核销该订单")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "openId", value = "openId",
                    paramType ="query" ,required = true,dataType = "string",defaultValue = ""),
    ...
    @ApiOperation(value="支付成功后通知后台（移动pos调用的）", notes="支付成功后通知后台（移动pos调用的）")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "openId", value = "openId",
                    paramType ="query" ,required = true,dataType = "string",defaultValue = ""),
            @ApiImplicitParam(name = "merchantOrderId", value = "订单号",
    ...
    @ApiOperation(value="支付成功后通知后台（前台pos调用的）", notes="支付成功后通知后台（前台pos调用的）")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "openId", value = "openId",
                    paramType ="query" ,required = true,dataType = "string",defaultValue = ""),
            @ApiImplicitParam(name = "merchantOrderId", value = "订单号",
    ...
    @ApiResponses({
            ...
            @ApiResponse(code = 404, message = "服务器找不到给定的资源；文档不存在"),
            @ApiResponse(code = 500, message = "服务器不能完成请求")})
    @RequestMapping(value = "/api/confirmPayedCash", method = RequestMethod.POST)
    @ResponseBody
    public  String confirmPayedCash(
            @RequestParam(value = "openId",required = true) String openId,
            @RequestParam(value = "merchantOrderId",required = true) String merchantOrderId,
            @RequestParam(value = "payOrderId",required = true) String payOrderId,
            @RequestParam(value = "amount",required = true) String amount,
        ...
        log.info("线下核销上传的数据 openId:{}, merchantOrderId:{},amount:{}, extraInfo:{}, storeName:{} "
                ,openId,merchantOrderId,amount,extraInfo,storeName);
        return this.smgService.payLog(openId,merchantOrderId,amount,extraInfo,payOrderId,storeId,storeName,2,2);
    }
}
